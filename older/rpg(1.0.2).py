...
def sell(co,ile=1):
    ...
    if co in eq:
            items-=int(ile)
            eqamount[eq.index(co)]-=int(ile)
            money+=(cena[cena.index(co)+1])*int(ile)
            printc(f"Sprzedano {co} * {ile} za {(cena[cena.index(co)+1])*int(ile)}$", color=Fore.GREEN)
def give(co,ile=1):
    global items
    global slots
    global eq
...
def ak():
    ...
    if akcja == "pomoc":
        ...
        a=1
        ile = akcja[akcja.index(" ")+a]
        while akcja[akcja.index(" ")+a]!=" ":
            a+=1
            ile = akcja[(akcja.index(" ")+1):(akcja.index(" ")+a)]
        inuse = akcja[+(akcja.index(" ")+a+1):]
        if inuse in legendarny or inuse in niezwykły or inuse in pospolity:
            if not inuse in eq or eqamount[eq.index(inuse)]==0:
                printc(f"Nie masz tego przedmiotu!",color=Fore.RED)
            elif inuse not in eq or int(eqamount[eq.index(inuse)])<int(ile):
                printc(f"Nie masz odpowiedniej ilości tego przedmiotu!",color=Fore.RED)
        ...
        if inuse in legendarny or inuse in niezwykły or inuse in pospolity:
            ...
            elif (items+int(ile))>slots:
                printc(f"Nie masz miejsca w ekwipunku!",color=Fore.RED)
            elif money<int(cena[cena.index(inuse)+1])*int(ile)*2:
                printc(f"Nie stać cię!",color=Fore.RED)
            else:
                give(inuse)
                money-=cena[cena.index(inuse)+1]*2*int(ile)
                printc(f"Kupiono {inuse} * {ile} za {cena[cena.index(inuse)+1]*2*int(ile)}$", color=Fore.GREEN)
        else:
            printc(f"Nie ma takiego przedmiotu!", color=Fore.RED)
    elif akcja=="$cls":
        print("\033c", end='')
    elif akcja=="$money_get":
        print(f"{money}")
    elif akcja[:10]=="$money_set":
        money=akcja[+11:]
        money=int(money)
    elif not akcja[:1]=="$":
