...
cena=["chleb",8, "skóra",20, "śledź",11, "łosoś",15,"karp",13,"diamenty",1500, "pół złota rybka","-","złoto",200,"węgorz",25]

exptolvl = [20, 35, 60, 90, 140, 200, 250, 350, 500, 575, 675, 750, 850, 1000] #5495 xp suma
legendarny = ["diamenty", "pół złota rybka"]
niezwykły = ["złoto", "węgorz"]
pospolity = ["chleb", "skóra", "śledź", "łosoś", "karp"]

itemy = ["wędkowaniepospolite", "wędkowanieniezwykłe", "wędkowanielegendarne"]
ryby = ["śledź", "karp", "łosoś", "węgorz", "pół złota rybka"]

wędkowaniepospolite = ["chleb", "skóra"]
wędkowanieniezwykłe = ["złoto"]
wędkowanielegendarne = ["diamenty"]

wędkowanie = ["ryby", "itemy"]
...
def ak():
    ...
    if akcja == "pomoc":
        ...
        if drop == "ryby":
            ...
            drop = random.choices(itemy, weights=(78, 24, 6), k=1)[0]
    ...
    if najedzenie<0 and akcja=="wędkowanie":
        ...
        printc(f"Staciłeś {((abs(najedzenie))/10)} punktów hp z powodu głodu!(Teraz: {hp}/{maxhp})",color=Fore.RED)
    if hp<0:
        print("\033c", end='')
        a = input("Umarłeś!\n")
        sys.exit()
